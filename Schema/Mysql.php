...
{
    // MySQL/MariaDB uses DROP FOREIGN KEY to remove a foreign key constraint by name,
    // then DROP COLUMN to remove the column. Ensure the constraint name matches the
    // one created above: definition_of_done_user_id
    $pdo->exec("
    ALTER TABLE definition_of_done
    DROP FOREIGN KEY definition_of_done_user_id
    ;");
